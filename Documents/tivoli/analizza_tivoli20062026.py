...
PREV_DEF     = os.path.join(DOWNLOADS, "tivoli17062026_output", "buonipasto DEFINITIVO")
OUT_ROOT     = os.path.join(DOWNLOADS, "tivoli20062026_output")
DEFINITIVO   = os.path.join(OUT_ROOT, "buonipasto DEFINITIVO")
OUT_ZIP      = os.path.join(DOWNLOADS, "tivoli20062026_DEFINITIVO.zip")
BUONO_EURO   = 4.13
BUONO_ORE    = 0.5
...
def process_worker(zip_path, worker, xlsx_name=None, inner_zip_name=None):
    print(f"\n{'='*55}")
    ...
    with zipfile.ZipFile(zip_path) as z:
        ...
        pdf_source_bytes = z.read(inner_zip_name) if inner_zip_name else None
    ...
    if pdf_source_bytes is None:
        with open(zip_path, 'rb') as f:
            pdf_source_bytes = f.read()
    pdfs = collect_pdfs_recursive(pdf_source_bytes)
...
WORKERS = [
    ("battistigiannitivoli.zip",                  "BATTISTI GIANNI", "BATTISTI GIANNI.xlsx", "cartellini 15 -23.zip"),
    ("CARTELLINI.zip",                             "RAIOLA GIOVANNA", None,                   None),
    ("lirosigiuliaelovalloangeladaconteggiare.zip","LI ROSI GIULIA",  None,                   "CARTELLINI 2020 - 2026.zip"),
    ("lirosigiuliaelovalloangeladaconteggiare.zip","LOVALLO ANGELA",  None,                   "CARTELLINI LOVALLO.zip"),
]
