...
PREV_DEF    = os.path.join(DOWNLOADS, "tivoli01072026_output", "buonipasto DEFINITIVO")  # 213 workers
OUT_ROOT    = os.path.join(DOWNLOADS, "tivoli02072026_output")
DEFINITIVO  = os.path.join(OUT_ROOT, "buonipasto DEFINITIVO")
OUT_ZIP     = os.path.join(DOWNLOADS, "tivoli02072026_DEFINITIVO.zip")
BUONO_EURO  = 4.13
BUONO_ORE   = 0.5
...
# ── Carry-forward dal DEFINITIVO 01/07 (213 worker) ──────────────────────────
print("Carry-forward da DEFINITIVO 01/07/2026...")
...
def process_worker(worker, zip_sources, base_records=None):
    print(f"\n{'='*55}")
...
TILIA_ZIP  = os.path.join(DOWNLOADS, "tiliaemanuelativoli.zip")
ABBATI_ZIP = os.path.join(DOWNLOADS, "abbatimariativoli.zip")

# TILIA EMANUELA (Matricola 40585, diversa da ATTILIA EMANUELA gia' in archivio,
# confermato via cedolino: Cognome ATTILIA CF TTLMNL67T42B595F Matricola 10025)
# nuova al bacino: base = xlsx nello zip (2016-2023,2025), poi PDF mensili 2024
with zipfile.ZipFile(TILIA_ZIP) as z:
    tilia_base = read_xlsx_records(z.read("TILIA EMANUELA.xlsx"), "TILIA EMANUELA")
process_worker("TILIA EMANUELA",
               [(TILIA_ZIP, None)],
               base_records=tilia_base)

# ABBATI MARIA: del tutto nuova, solo PDF (due zip annidati: 2015-2023 + 24-25)
process_worker("ABBATI MARIA",
               [(ABBATI_ZIP, None)])
