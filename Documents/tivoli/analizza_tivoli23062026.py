...
PREV_DEF     = os.path.join(DOWNLOADS, "tivoli20062026_output", "buonipasto DEFINITIVO")
OUT_ROOT     = os.path.join(DOWNLOADS, "tivoli23062026_output")
DEFINITIVO   = os.path.join(OUT_ROOT, "buonipasto DEFINITIVO")
OUT_ZIP      = os.path.join(DOWNLOADS, "tivoli23062026_DEFINITIVO.zip")
BUONO_EURO   = 4.13
BUONO_ORE    = 0.5
...
def process_worker(worker, zip_sources, xlsx_source=None):
    """
    zip_sources: lista di (zip_path, path_filter_o_None). path_filter filtra per
                 sottostringa del path interno allo zip (case-insensitive) — serve
                 per isolare un lavoratore da uno zip condiviso multi-worker.
    xlsx_source: (zip_path, path_interno_xlsx) per leggere lo storico pregresso, o None.
    """
    print(f"\n{'='*55}")
    ...
    pdfs = []
    for zpath, pfilter in zip_sources:
...
# ── Processa i nuovi lavoratori ────────────────────────────────────────────

GT       = os.path.join(DOWNLOADS, "Gianmarco tivoli.zip")
GT2      = os.path.join(DOWNLOADS, "gianmarco tivoli 2.zip")
LOMBARDI = os.path.join(DOWNLOADS, "Lombardi Delia.zip")
PANUNZI  = os.path.join(DOWNLOADS, "Panunzi Giovanna.zip")
TAB1     = os.path.join(DOWNLOADS, "TABULATO 2015-2025-1-70 (1).zip")
TAB2     = os.path.join(DOWNLOADS, "TABULATO 2015-2025-71-133.zip")

WORKERS = [
    ("ROSATI ALESSIO",               [(GT,  "Rosati Alessio")],  None),
    ("SALVATI SIMONA",               [(GT,  "Salvati Simona")],  None),
    ("SCALABRINO FABIOLA JOSEFINA",  [(GT,  "Scalabrino Fabiola")],
        (GT, "Gianmarco tivoli/Scalabrino Fabiola/SCALABRINO FABIOLA JOSEFINA.xlsx")),
    ("TANONI TERESA",                [(GT,  "Tanoni Teresa")],
        (GT, "Gianmarco tivoli/Tanoni Teresa/TANONI TERESA.xlsx")),
    ("BUGARA ZUZANNA MARIA",         [(GT2, "Bugara Zuzanna Maria")], None),
    ("GENNARI MARIA CHIARA",         [(GT2, "Gennari Maria Chiara")], None),
    ("LOMBARDI DELIA",               [(LOMBARDI, None)], None),
    ("PANUNZI GIOVANNA",             [(PANUNZI, None)],  None),
    ("MARRACCINI CINZIA",            [(TAB1, None), (TAB2, None)], None),
]

for worker, zip_sources, xlsx_source in WORKERS:
    process_worker(worker, zip_sources, xlsx_source)
