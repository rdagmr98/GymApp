...
1. Aggiornare il hub del progetto (stato, decisioni, TODO) → sezione "STATO SESSIONE", SOLO ultime 1-2 voci (rolling window, comprimere/rimuovere le più vecchie quando si aggiunge la nuova)
2. Nessun changelog cronologico separato da mantenere (`Sessioni/YYYY-MM-DD.md` e "Ultime sessioni" in `_CLAUDE.md` sono stati eliminati come obbligo il 2026-08-28 — l'utente non li legge mai per cronologia; la cronologia reale, se serve, è il `git log` del vault). Il valore centrale resta la nota atomica (STEP 2.5) e lo stato-progetto nell'hub.
...
- Ogni nota ha `tipo:` nel frontmatter YAML — 3 valori standing: `hub` (MOC), `regole` (checklist standing, vedi STEP 2 sopra), `atomica` (un fatto/bugfix/decisione). Il tipo `sessioni` (changelog cronologico) è stato eliminato come obbligo il 2026-08-28 — l'utente non legge mai le note per cronologia, la cronologia reale è il `git log` del vault. Serve a leggere solo il set minimo di note pertinenti a un task invece dell'intero progetto. Tabella completa: `ObsidianVault/_CLAUDE.md` → sezione "Tipi di nota".
...
| autom.lab (sito vetrina/lead-gen brand personale) | `C:\Users\Gianmarco\autom.lab` → GitHub `rdagmr98/autom.lab`, live `rdagmr98.github.io/autom.lab` (deploy via Actions) | `autom.lab/autom.lab.md` (Obsidian) |
