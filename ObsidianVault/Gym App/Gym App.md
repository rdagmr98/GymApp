...
- **GymApp web — causa VERA del freeze/white-screen iPhone trovata e risolta: deploy fermo a build 18 + regressione `viewport-fit=cover`** (19 giu 2026, corregge la voce sotto): l'utente ha riportato che il freeze persisteva identico anche dopo il "fix" del timer cardio, e su Safari puro è apparso pure **schermo bianco**. Controllo a fondo (richiesto esplicitamente dall'utente: "controlla co'hai cambiato dalla versione di mercoledì 10 giugno che funzionava benissimo") ha rivelato due problemi distinti:
  ...
  - **Fix**: rimossa la riga viewport da `web/index.html` e `index.html`, rebuild `flutter build web --release --base-href "/GymApp/"`, commit `924bdbd` su `main` (porta finalmente live tutti gli 8 commit pendenti, incluso il fix cardio). Verificato live: `version.json` → build 20, nessun meta viewport nell'HTML servito, `gh api .../pages/builds/latest` → `"status":"built"`.
  - **Deploy chiarito** (vedi sezione Build e Deploy sopra, corretta): Pages source reale = branch `main`, path `/` (verificato via `gh api repos/rdagmr98/GymApp/pages`). Questo repo NON deploya su `gh-pages` (quel branch esiste ancora sul remote ma è orfano/inutilizzato) — la voce sotto che parla di "web ridepl. su gh-pages" era imprecisa.
  - **Nessuna nuova GitHub Release**: APK/AAB rebuilt sono identici a `v1.0.2-20` (già esistente, stesso source dart) — non duplicata.
  - **Da verificare dall'utente**: testare su iPhone reale (Safari) che il freeze/white-screen sia ora risolto — non testabile da remoto, questa è la parte ancora da confermare.
- **GymApp web — root cause ~~VERO~~ del freeze iPhone trovato e ~~risolto~~** (19 giu 2026, **release web non era mai realmente live — vedi voce sopra**): il fix del wasm stale (18 giu, vedi sotto) NON era la causa reale — l'utente ha confermato che dopo quel deploy il freeze persisteva identico ("ad ogni tocco risponde dopo mezzo minuto", solo su iPhone, mai su Android/Windows, mai su gymapplogbook/fix-ads che non ha questo problema).
  - **Causa reale**: leak di `Timer.periodic` nel countdown cardio (`_cardioCountdownTimer`, esclusivo di gym_app — fix-ads non ha la feature cardio). Veniva cancellato solo a fine countdown naturale o in `dispose()`, ma MAI quando si abbandonava l'esercizio cardio a metà (skip rest, cambio esercizio, fine round, fine workout, uscita anticipata). Ogni cardio abbandonato lasciava un timer orfano attivo nello stesso State, che continuava a fare `setState()` ogni secondo per tutta la sessione. Più cardio abbandonati = più timer accumulati = più `setState`/sec. Su Safari/CanvasKit (più lento di Chrome nel rendering) questo saturava il render thread fino a bloccare ogni tap per decine di secondi. (Bug reale e fix corretto — il problema era che non era l'UNICA causa, vedi voce sopra.)
  ...
  - **Release**: ~~`v1.0.2+20` — web ridepl. su `gh-pages`~~ **IMPRECISO**: il web in realtà NON era stato ridepl. (vedi voce sopra — fermo a build 18 fino al fix successivo). APK arm64/armeabi/x86_64 + AAB → GitHub Release `v1.0.2-20` (questa parte era corretta).
  - **Lezione**: comparare sempre col comportamento di fix-ads quando un bug è "solo su una delle due app" — la feature assente nell'altra app è il primo sospetto, non l'ambiente/deploy. **Lezione aggiuntiva**: dopo un fix web, verificare SEMPRE il deploy live (`curl .../version.json`) prima di dichiararlo risolto — non basta il commit.
...
- **Web**: GymApp → **branch `main` root** (vedi sezione Build e Deploy sopra — corretto 19 giu, non più gh-pages), AppCliente → gh-pages (`/tmp/gymapplogbook-gh-pages`)
