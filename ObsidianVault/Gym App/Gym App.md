...
- `gym_app-1.0.2+29-arm64-v8a.apk` / `-armeabi-v7a.apk` / `-x86_64.apk`
- `gym_app-1.0.2+29.aab`
...
- **Pulsante "Salva serie" invisibile S23 — causa reale: MediaQuery inset inaffidabile Samsung One UI, fix floor 64px** (07 lug 2026, v1.0.2+28): dopo 2 tentativi falliti (fix layout strutturale Row fuori scroll, poi MediaQuery dinamico) causa vera trovata: `MediaQuery.of(ctx).padding.bottom` sottostima l'inset nav bar su questo device anche in edge-to-edge. Fix: max tra inset OS e soglia fissa 64.0, applicato a gym_app e app_cliente. **Ancora da confermare dall'utente su S23 reale** (4° tentativo) — se fallisce ancora, chiedere screenshot con "mostra limiti layout" invece di indovinare valori.
