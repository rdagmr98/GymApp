#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import os, sys, zipfile, io, tempfile, re
...
def parse_cid29_pdf(pdf_path):
    ...
    month = _MESI_IT.get(m.group(2).upper())
    year = int(m.group(3))
    if not month:
        return None
...
EXISTING   = os.path.join(DOWNLOADS, "tivoli11062026", "buonipasto DEFINITIVO", "PICCHI ALESSIA.xlsx")
...
print(f"Lettura Excel esistente: {EXISTING}")
existing_records = []
wb = openpyxl.load_workbook(EXISTING, read_only=True, data_only=True)
...
print(f"  Mesi esistenti: {len(existing_records)}  anni={sorted({r['year'] for r in existing_records})}")

# 2. Processa PDF dal nuovo ZIP
zpath = os.path.join(DOWNLOADS, "PICCHI ALESSIA.zip")
new_records = []
with zipfile.ZipFile(zpath) as z:
    pdfs = sorted(n for n in z.namelist() if n.lower().endswith('.pdf'))
    print(f"PDF da processare: {len(pdfs)}")
    for pname in pdfs:
        pdata = z.read(pname)
...
print(f"Record dai PDF: {len(new_records)}")
for r in sorted(new_records, key=lambda x: (x['year'], x['month'])):
    print(f"  {r['year']}/{r['month']:02d}  mat={r['maturati']} ero={r['erogati']}")

# 3. Merge
truly_new = [r for r in new_records if (r['year'], r['month']) not in existing_keys]
print(f"\nNuovi mesi: {sorted((r['year'], r['month']) for r in truly_new)}")
...
print(f"Totale: mat={tot_mat} ero={tot_ero} delta={tot_mat-tot_ero} ({(tot_mat-tot_ero)*BUONO_EURO:.2f}EUR)  anni={anni}")
...
workers2 = sorted(agg.keys())
years2   = sorted({y for w in agg.values() for y in w})
print(f"  Lavoratori: {len(workers2)},  Anni: {years2[0]}-{years2[-1]}")

wb3 = openpyxl.Workbook(); ws3 = wb3.active; ws3.title = "Riepilogo Generale"
rh(ws3, 1, 1, "Lavoratore")
for ci, y in enumerate(years2, 2): rh(ws3, 1, ci, y)
tc = len(years2) + 2; ec = len(years2) + 3; oc = len(years2) + 4
tcl = openpyxl.utils.get_column_letter(tc)
lyl = openpyxl.utils.get_column_letter(len(years2) + 1)
...
for ri, w2 in enumerate(workers2, 2):
    fill = ALT_FILL if ri % 2 == 0 else None
    rc(ws3, ri, 1, w2, fill=fill, align=LFT)
    for ci, y in enumerate(years2, 2):
        v = agg[w2].get(y, 0)
    ...
    ero_tot  = sum(agg_ero[w2].get(y, 0) for y in years2)
...
for ci in range(2, len(years2) + 5):
...
with zipfile.ZipFile(OUT_ZIP, "w", zipfile.ZIP_DEFLATED) as zout:
    for fname in all_xlsx:
        ...
        print(f"  + {fname}")
print(f"ZIP aggiornato: {OUT_ZIP}")
